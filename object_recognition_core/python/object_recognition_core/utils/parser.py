...
class ObjectRecognitionParser(argparse.ArgumentParser):
    ...
    def remove_launchfile_generated_args(self, arg_strings):
        ...
        for arg_string in arg_strings:
            if not arg_string.startswith('__name:=') and not arg_string.startswith('__log:=') and not arg_string.startswith('/recognized_object_array:=') and not arg_string.startswith('recognized_object_array:='):
                new_arg_strings.append(arg_string)
        return new_arg_strings
